"""rffp.evaluation.verify.run_verify — VERIFICATION BATTERY V1-V5 (build+verify session).

  python3 -m rffp.evaluation.verify.run_verify
...
from rffp.data.unified import UnifiedRFDataset, DomainHomogeneousBatchSampler, collate_domain
from rffp.models.gen_encoder import GenRFEncoder, param_count
from rffp.physics.features import classical_matrix, residual_batch
from rffp.evaluation.bench import lopo
from rffp.config import RESULTS_DIR as RESULTS_GEN, REPO_ROOT as _DLM
...
def v4_physics(ds):
    ...
    Xd = Xt[:100]
    A = classical_matrix(Xd)
    # Optional cross-check against the independent step-4 reference implementation, which lives
    # in the extended research tree (mechanism_validation) and is NOT shipped with the release.
    try:
        ...
        print("  P1-G1 cross-check SKIPPED (mechanism_validation reference not in release tree)")
...
def v5_model(ds):
    ...
    from rffp.evaluation.bench.harness import knn_purity
