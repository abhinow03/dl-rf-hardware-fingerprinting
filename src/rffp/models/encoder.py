"""rffp.models.encoder — the dual-branch RFEncoder (the shared fingerprint backbone).

A ~1.5M-parameter dual-stream encoder that maps a raw I/Q window to a device-fingerprint
embedding:

    TimeBranch     1-D CNN over the raw waveform  [B,2,L]      -> 256-D
    SpectralBranch 2-D CNN over the STFT magnitude [B,2,F,T]   -> 256-D
    cross-attention fuses them (query=time, key/value=spectral) + residual
    LayerNorm(512) -> get_encoder_output() : 512-D fusion embedding
    projection head -> forward()           : 128-D L2-normalized metric embedding

Both branches use GroupNorm + Squeeze-and-Excitation residual blocks. Reference window is
4096 samples with STFT (n_fft=256, hop=64, Hann) -> [129, 61] per channel, but the encoder is
shape-flexible via global pooling. `forward` is the contrastive metric head; `get_encoder_output`
is the richer pre-projection representation used by the deep tiers. Loaded read-only at inference.
"""
import torch
