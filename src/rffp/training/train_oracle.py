"""rffp.training.train_oracle — Phase-1 ORACLE closed-set trainer (RFEncoder + SupCon).

Trains the dual-branch RFEncoder on the ORACLE 16-device WiFi dataset using supervised
contrastive learning. Trains only on 8ft + 14ft (high-SNR, avoids distance leakage);
4096-sample windows (819.2 us @ 5 MS/s) capture multiple I/Q-imbalance cycles.

    python3 -m rffp.training.train_oracle

Dataset / checkpoint paths resolve via rffp.config (RFFP_ORACLE, RFFP_ORACLE_CKPT).
"""
import os
import json
import random
